...
def _axis_label(self, datasets) -> str:
    if self._button_checked(self.btn_phase):
        return "Phase (deg)"
    if self._plot_scale_is_linear():
        return "RCS (Linear)"
    ...
    if len(units) == 1:
        return f"RCS ({next(iter(units))})"
    return "RCS (dB)"


def _residual_unit(self, datasets) -> str:
    if self._button_checked(self.btn_phase):
        return "deg"
    if self._plot_scale_is_linear():
        return "Linear"
    ...
    if sweep_axis == "azimuth":
        ...
        for f_idx in freq_indices:
            ...
            for e_idx in elev_indices:
                ...
                rcs_display = _display_from_values(self, dataset, raw, frequency_value=freq_value)
        ...
        for e_idx in elev_indices:
            ...
            for a_idx in az_indices:
                ...
                rcs_display = _display_from_values(
                    self, dataset, raw, frequency_value=freq_axis_values
                )
...
def render(self) -> None:
    ...
    top_ax.set_ylabel(_axis_label(self, datasets))
    ...
    if mask_a.sum() < 2:
        ...
        residual_unit = _residual_unit(self, datasets)
